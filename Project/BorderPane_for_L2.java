...
public class BorderPane_for_L2 extends BorderPane {
	...
	Bottom_Board2 bottom_board2 = new Bottom_Board2();
...
class Game_Board2 extends GridPane {
	...
	public void build() {
		...
		for(int i = 0;i<10;i++) {
			for(int j = 0;j<10;j++) {
				...
				top_board2.getChildren().clear();
				top_board2.getChildren().add(name);
				top_board2.getChildren().add(point);
				top_board2.getChildren().add(score);
		...
		//These are just settings for level-2. 
		r_list[1][4].setFill(Color.PURPLE);r_list[1][4].setDestroyable(true);
		r_list[2][4].setFill(Color.PURPLE);r_list[2][4].setDestroyable(true);
		r_list[3][4].setFill(Color.PURPLE);r_list[3][4].setDestroyable(true);
		r_list[4][5].setFill(Color.PURPLE);r_list[4][5].setDestroyable(true);
		r_list[5][4].setFill(Color.PURPLE);r_list[5][4].setDestroyable(true);
		r_list[6][4].setFill(Color.PURPLE);r_list[6][4].setDestroyable(true);
		r_list[7][4].setFill(Color.PURPLE);r_list[7][4].setDestroyable(true);
		...
		r_list[4][6].setFill(Color.BLUE);r_list[4][6].setDestroyable(true);
		...
		r_list[5][5].setFill(Color.BLUE);r_list[5][5].setDestroyable(true);
		r_list[5][6].setFill(Color.BLUE);r_list[5][6].setDestroyable(true);
		r_list[5][7].setFill(Color.BLUE);r_list[5][7].setDestroyable(true);
		
		
		r_list[1][1].setFill(Color.WHITE);
		r_list[8][8].setFill(Color.WHITE);
		r_list[1][8].setFill(Color.WHITE);
		r_list[8][1].setFill(Color.WHITE);
	...
	public void destroy_all(rectangle r) {
		...
		if(i==0 && j==0 && r.IsDestroyable() ) {
			if(r_list[i][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().clear();
				bottom_board2.getChildren().add(box);
			}
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit1);
			}
			if(r_list[i][j+1].IsDestroyable()) {
				r_list[i][j+1].destroy();
				counter +=1;
				Label hit2 = new Label("					Hit:" + i +"-" + (j+1));
				bottom_board2.getChildren().add(hit2);
				}
			//according to counter value we need to print bottom page the value of gained or lost points
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
				bottom_board2.getChildren().add(new Label("															(-1 points)"));
			else if (counter == 3)
				bottom_board2.getChildren().add(new Label("															(+1 points)"));
			else if (counter ==4)
				bottom_board2.getChildren().add(new Label("															(+2 points)"));
			else if (counter == 5)
				bottom_board2.getChildren().add(new Label("															(+4 points)"));
			...
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit3);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit4 = new Label("					Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit4);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i-1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit5);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit6 = new Label("					Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit6);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i-1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit7);
				}
			if(r_list[i][j+1].IsDestroyable()) {
				r_list[i][j+1].destroy();
				counter +=1;
				Label hit8 = new Label("						Hit:" + i +"-" + (j+1));
				bottom_board2.getChildren().add(hit8);
				}
			...
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit9);
				}
			if(r_list[i][j+1].IsDestroyable()) {
				r_list[i][j+1].destroy();
				counter +=1;
				Label hit10 = new Label("					Hit:" + i +"-" + (j+1));
				bottom_board2.getChildren().add(hit10);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit11 = new Label("				Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit11);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit12);
				}
			if(r_list[i-1][j].IsDestroyable()) {
				r_list[i-1][j].destroy();
				counter +=1;
				Label hit13 = new Label("						Hit:" + (i-1) +"-" + j);
				bottom_board2.getChildren().add(hit13);
				}
			if(r_list[i][j+1].IsDestroyable()) {
				r_list[i][j+1].destroy();
				counter +=1;
				Label hit14 = new Label("								Hit:" + i +"-" + (j+1));
				bottom_board2.getChildren().add(hit14);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i][j+1].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit15);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit16 = new Label("						Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit16);
				}
			if(r_list[i-1][j].IsDestroyable()) {
				r_list[i-1][j].destroy();
				counter +=1;
				Label hit17 = new Label("								Hit:" + (i-1) +"-" + j);
				bottom_board2.getChildren().add(hit17);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit18);
				}
			if(r_list[i-1][j].IsDestroyable()) {
				r_list[i-1][j].destroy();
				counter +=1;
				Label hit19 = new Label("						Hit:" + (i-1) +"-" + j);
				bottom_board2.getChildren().add(hit19);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit20 = new Label("								Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit20);
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
			else if (counter == 2)
			...
			if(r_list[i+1][j].IsDestroyable()) {
				...
				bottom_board2.getChildren().add(hit21);
				}
			if(r_list[i-1][j].IsDestroyable()) {
				r_list[i-1][j].destroy();
				counter +=1;
				Label hit22 = new Label("						Hit:" + (i-1) +"-" + j);
				bottom_board2.getChildren().add(hit22);
				}
			if(r_list[i][j-1].IsDestroyable()) {
				r_list[i][j-1].destroy();
				counter +=1;
				Label hit23 = new Label("								Hit:" + i +"-" + (j-1));
				bottom_board2.getChildren().add(hit23);
				}
			if(r_list[i][j+1].IsDestroyable()) {
				r_list[i][j+1].destroy();
				Label hit24 = new Label("										Hit:" + i +"-" + (j+1));
				bottom_board2.getChildren().add(hit24);
				counter +=1;
				}
			if (counter == 1)
				bottom_board2.getChildren().add(new Label("															(-3 points)"));
      ...
      //When our level is completed , we print the next level is available now! message to the bottom page.
        if(is_it_done == true && r_count == 100) {
            ...
            bottom_board2.setPadding(new Insets(30,30,30,30));
            bottom_board2.getChildren().add(nextLevel);
            nextLevel.setLayoutX(220);
            nextLevel.setLayoutY(13);
         
        }
	}//end of the destroy all function.
}// end of the game board 2.

//**************************************************************************
//This is the top board of the level-2
class Top_Board2 extends HBox{
...
//**************************************************************************
//This is the bottom board of the level-2
class Bottom_Board2 extends Pane{
	
	public Bottom_Board2() {
		build();
	}
